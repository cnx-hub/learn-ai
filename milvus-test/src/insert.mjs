...
// console.log(import.meta.url, fileURLToPath(import.meta.url), __dirname, '__dirname')
...
async function main() {
    try {
        console.log('Milvus Client is connected');
        ...
        const diaryData = await Promise.all(diaryContents.map(async (item) => {
            return {
                ...item,
