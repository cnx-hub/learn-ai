...
const routerQuestionNode = async (state) => {
    try {
        ...
        console.error("路由问题时出错:", error.message);

        return {
            strategy: 'simple',
            routeReason: '路由失败，默认使用简单策略',
        }
    }
...
const directAnswerNode = async (state) => {
    try {
        ...
        return {
            generation: '抱歉，我无法回答这个问题。',
        }
    }
};

const retrieveLocalNode = async (state) => {
    console.log("---RETRIEVE_LOCAL---");
...
const graph = new StateGraph(stateGraph)
    .addNode('router_question', routerQuestionNode)
    .addNode('local_search', retrieveLocalNode)
    .addNode('evaluate', evaluateNode)
    .addNode("web_search", webSearchNode)
    .addNode('direct_answer', directAnswerNode)
    .addNode('generate', generateNode)
    .addEdge(START, 'router_question')
    .addConditionalEdges('router_question', afterRoute, {
        direct_answer: 'direct_answer',
        local_retrieve: 'local_search',
    })
    .addEdge('local_search', 'evaluate')
    .addConditionalEdges('evaluate', afterEvaluateLocal, {
        generate: 'generate',
