...
export class TtsRelayService implements OnModuleDestroy {
    ...
    registerClient(clientWs: WebSocket.WebSocket, wantedSessionId?: string): string {
    ...
    private ensureTencentConnection(session: ClientSession) {
        if (session.tencentWs && session.tencentWs.readyState <= WebSocket.OPEN) {
            return;
        }
        ...
        const tencentWs = new WebSocket.WebSocket(url);
    ...
    private sendClientJson(clientWs: WebSocket.WebSocket, payload: Record<string, unknown>) {
    ...
    private closeSession(sessionId: string, reason: string) {
        ...
        if (session.clientWs && session.clientWs.readyState < WebSocket.CLOSING) {
    ...
    private buildTencentTtsWsUrl(sessionId: string) {
