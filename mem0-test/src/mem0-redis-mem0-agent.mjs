...
class RedisMessageStore {
    ...
    async loadMessages(sessionId) {
        const raw = await this.redis.get(this.messageKey(sessionId));
        if (!raw) return [];
        // TODO 查看一下 raw 原始的值是什么
    ...
    async saveMessages(sessionId, messages) {
        ...
        await this.redis.set(this.messageKey(sessionId), payLoad, "EX", this.ttlSeconds)
    }

    async clear(sessionId) {
        await this.redis.del(this.messageKey(sessionId));
    }

    async ttl(sessionId) {
        return await this.redis.ttl(this.messageKey(sessionId));
    }
}
...
class Mem0MemoryStore {
    ...
    async classifyAndPersist(userText, assistantText) {
        ...
        // write_user, write_session 这两个作用
