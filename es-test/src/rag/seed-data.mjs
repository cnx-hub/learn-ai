// import { Client } from "@langchain/community/clients/es";
...
async function seedMilvus(collectionName, rows, emb) {
    try {
        ...
        console.error('[Milvus 错误]', error.message);
...
async function main() {
    try {
        ...
        await seedMilvus(INDEX, ROWS, embeddings);
        await seedElasticsearch(INDEX, ROWS);
    } catch (error) {
        console.error('\n错误:', error.message);
        console.error(error.stack);
        process.exit(1);
